...
jQuery(document).ready(function($) {
  $('.project-subtitle').each(function() {
    var text = $(this).text().trim().replace(/\u00a0/g, '');
    if (text === '') {
      $(this).closest('.Project_detail_page').addClass('remove_all_sec');
    }
  });
});
...
(function ($) {
  ...
  // });
// Detect device and remove unused video when modal is shown
  $('#videoModal').on('shown.bs.modal', function () {
    if ($(window).width() <= 768) {
      // Mobile → remove desktop video
      $('.desktop__modal').remove();
      $('.mobile__modal')[0].play();
    } else {
      // Desktop → remove mobile video
      $('.mobile__modal').remove();
      $('.desktop__modal')[0].play();
    }
  });
